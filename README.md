...
### Phase 2.5 新增（多内核隔离与流式执行）
- ✅ **持久子内核进程**: engine-backed Python 使用常驻 `kernel_worker.py` 进程，通过 JSON-over-STDIO 与主服务通信
- ✅ **进程间通信 (IPC)**: 实现了 JSON-over-STDIO 协议，支持心跳（ping/pong）和超时控制
- ✅ **健康监控**: 为每个子内核启动监控任务，定时发送心跳；若进程崩溃自动重启
- ✅ **WebSocket 流式输出**: 前端通过 WebSocket 接收实时执行输出，REST 回退机制
- ✅ **状态栏指示**: 顶部显示内核连接状态，底部显示活跃语言/引擎/VEnv

### 后端功能
- ✅ FastAPI REST API + WebSocket 长连接
- ✅ 会话管理（创建、查询、删除）
- ✅ 多语言代码执行引擎
- ✅ 虚拟环境创建和管理（3 个独立隔离的 Python venv）
- ✅ 完整错误处理和异常捕获
- ✅ 异步任务执行
- ✅ **持久化子内核进程**（JSON-over-STDIO IPC，基于 `kernel_worker.py`）
- ✅ **心跳监控与自动重启**（定期 ping/pong 检测子进程健康状况，故障自动重启）
- ✅ **流式输出支持**（WebSocket 实时推送执行结果和日志）
...
- ✅ 结果显示面板
- ✅ 响应式布局设计
- ✅ **WebSocket 流式输出支持**（实时显示执行过程中的输出）
- ✅ **内核/VEnv 状态栏**（顶部显示内核连接状态，底部显示语言/引擎/VEnv 信息）
